...
const postTodo = async (req, res) => {
    try {
        ...
        return res.status(201).json({description, urgency, creationDate, completed});
    } catch (error) {
        res.status(500).send("Internal Server Error");
    }
}
...
const putTodo = async (req, res) => {
    try {
        const {id} = req.params;
        const {description, urgency, creationDate, completed} = req.body;
        const todo = {description, urgency, creationDate, completed};
        ...
        return res.status(200).json({id, description, urgency, creationDate, completed});
    } catch (error) {
        res.status(500).send("Internal Server Error");
    }
}
